...
const PixelCanvas = ({ 
  ...
  useEffect(() => {
    ...
    if (selectedIndex !== null) {
      ...
      const selectedPixelImage = pixelImages.find(
        img => img.pixel.serial === pixels[selectedIndex]?.serial
      );
  ...
  const animateFrame = (timestamp) => {
    ...
    const newScale = animateToCenterPixel.startScale + 
      (animateToCenterPixel.targetScale - animateToCenterPixel.startScale) * eased;
  ...
  const drawGrid = () => {
    ...
    const selectedPixelImage = selectedIndex !== null ? 
    ...
    if (selectedPixelImage) {
      sortedPixelImages = sortedPixelImages.filter(
        img => img.pixel.serial !== selectedPixelImage.pixel.serial
      );
    }
